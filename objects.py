...
def is_point_below_line(x1, y1, x2, y2, x3, y3):
    # Вычисляем угловой коэффициент прямой
    if x2 - x1 == 0:
        return False
    m = (y2 - y1) / (x2 - x1)
    # Вычисляем b, точку пересечения с осью Y
    b = y1 - m * x1
    # Вычисляем y для x3 на прямой
    y_on_line = m * x3 + b
    # Сравниваем y третьей точки с y на прямой
    return y3 < y_on_line
...
class Oil:
    ...
    def draw(self, screen, current_time):
        ...
        self.mini_r = round(2 * self.s_y / math.cos(self.a) / self.zalupa)
        self.centre_of_circle = (round((self.s_x + math.tan(self.a) * 2 * self.s_y) / self.zalupa), 250)
        pg.draw.circle(screen, color.BLACK, self.centre_of_circle, self.mini_r)
        self.p1 = [0, 250]
        self.p2 = [0 + round(self.s_x / self.zalupa), 250 + 2 * round(self.s_y / self.zalupa)]
        self.p3 = [0 + round(self.s_x / self.zalupa), 250 - 2 * round(self.s_y / self.zalupa)]
...
class Fish(pg.sprite.Sprite):
    ...
    def update(self, oil: Oil | None):
        ...
        # oilCircleFlag = False
        # oilTringleFlag = False
        

        x = self.rect.centerx
        y = self.rect.centery
        
        circleX, circleY = oil.centre_of_circle
        circleRadius = oil.mini_r
        
        p1, p2, p3 = oil.p1, oil.p2, oil.p3
        
        if (x - circleX) ** 2 + (y - circleY) ** 2 <= circleRadius ** 2:
            self.changeImage()
            return
        
        if is_point_below_line(p1[0], p1[1], p3[0], p3[1], x, y):
            if not is_point_below_line(p1[0], p1[1], p2[0], p2[1], x, y):
                if x < p2[0]:
                    self.changeImage()
                    return
        ...
        # if is_point_in_triangle(Point(*p1), Point(*p3), Point(*p2), Point(x, y)):
        #     self.changeImage()
        #     return
        
        # if point_in_triangle(p1, p2, p3, (x, y)):
